...
INDEXES_WITHOUT_INTERVALS = ["accounts", "rating", "validators", "epochinfo", "tags", "delegators"]
    ...
    def plan_tasks_with_intervals(
        ...
        for index_name in INDEXES_WITH_INTERVALS:
            for start_timestamp in range(initial_start_timestamp, initial_end_timestamp, granularity_seconds):
                ...
                task = Task(id, indexer_url, index_name, start_timestamp, end_timestamp)
                tasks.append(task)

        return tasks

    def plan_tasks_without_intervals(self, indexer_url: str) -> List[Task]:
        tasks: List[Task] = []

        for index_name in INDEXES_WITHOUT_INTERVALS:
            id = self._next_task_id()
            task = Task(id, indexer_url, index_name)
            tasks.append(task)
