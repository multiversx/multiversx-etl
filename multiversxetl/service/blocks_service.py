...
class MultiversXBlocksService:
    ...
    def get_hyperblock_range_for_timestamps(self, start_timestamp: int, end_timestamp: int) -> Tuple[int, int]:
        ...
        start_hyperblock = self._get_hyperblock_nonce_by_timestamp(start_timestamp)
        end_hyperblock = self._get_hyperblock_nonce_by_timestamp(end_timestamp)
        ...
        return start_hyperblock, end_hyperblock

    def _get_hyperblock_nonce_by_timestamp(self, timestamp: int) -> Optional[int]:
        round = self._get_round_by_timestamp(timestamp)

        for _ in range(0, MAX_NUM_BLOCKS_LOOKAHEAD):
            nonce = self.provider.get_hyperblock_nonce_by_round(round)
            if nonce:
                return nonce
